...
def meteo_prod(sensor_alias, selected_comarca):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color='rgb(182, 199, 170)'),
        yaxis='y2'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color='rgb(160, 147, 125)'),
        yaxis='y1'
    ))

    fig.update_layout(
        title=f'{sensor_alias} y Producción en {selected_comarca} a lo largo de los años',
        xaxis_title='Año',
        yaxis=dict(
            title=leyendaSensor[sensor_alias],
            showgrid=True,
            side='left',
            layer="below traces"
        ),
